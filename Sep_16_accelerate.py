...
def get_all_data_large(split):
    ...
    x = torch.stack([data[i:i+block_size] for i in range(0, len(data) - block_size)])
    y = torch.stack([data[i+1:i+block_size+1] for i in range(0, len(data) - block_size)])
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.position_embedding_table(torch.arange(T, device=device).long()) # (T,C)
