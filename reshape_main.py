...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.output = ColumnParallelLinear(
            params.dim, 8, bias=False, init_method=lambda x: x # Changed output shape to 8
        )
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int, targets=None):
        ...
        # If you're aiming for mixed precision (i.e., float16):
        preds = preds.half() # Convert predictions to float16
        
        if targets is None:
            loss = None
        else:
            # Ensure that the targets are also in float16
            targets = targets.half()
            loss = F.binary_cross_entropy(preds.view(-1), targets)
