...
# data_val_iter = iter(dataloader_val)
...
for epoch in range(epochs):
    ...
    for xb, yb in dataloader_train:
        ...
        if (count % eval_interval == 0 or count == max_iters - 1) and local_rank==0:
            losses = estimate_loss()
            print(f"step {count}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")          
        
        output_tensor, loss = model(xb, targets=yb, start_pos=0)
...
context = torch.tensor(tokenizer.encode('H', False, False)).to(device).unsqueeze(0)
if local_rank == 0:
