...
X = torch.rand(4096, 3).half().to(device)
    ...
    def __init__(self, input_dims):
        ...
        with enable_wrap(**fsdp_params):
            ...
            self.model = nn.Sequential(
                wrap(nn.Linear(input_dims, dims)),
                wrap(nn.ReLU()),
                wrap(nn.Linear(dims, dims)),
                wrap(nn.ReLU()),
                wrap(nn.Linear(dims, 1))
            )
...
model = FSDP(model, mixed_precision=True, flatten_parameters=True)
